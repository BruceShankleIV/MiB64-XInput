...
namespace FileIO
{
	std::string GetFilename()
	{
		...
		const std::string plugin_filename("PJ64L_XInput.ini");
		...
		{
#ifdef _WIN32
			const std::string plugin_directory("config/");
	...
	bool FASTCALL LoadMempak(u8 *data, u8 port)
	{
		...
		sprintf(filename, "MemPaks\\PJ64L_XInput%d.mempak", port+1);
...
#ifdef _WIN32
		FILE *file = NULL;
		file = fopen("PJ64L_XInput.eeprom", "r");
...
#ifdef _WIN32
		FILE *file = NULL;
		file = fopen("PJ64L_XInput.eeprom", "w");
