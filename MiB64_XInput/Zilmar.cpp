...
#define ABOUT PLUGIN_NAME "\n" VERSION 
#define DLL_INFO PLUGIN_NAME VERSION 
#ifdef _WIN32
extern "C"
{
	HWND MainHwd = NULL;
...
{
	//Debug("PJ64L_XInput -> GetDLLInfo\n");

	strcpy(PluginInfo->Name, DLL_INFO);
...
DllExport void CALL InitiateControllers(HWND hMain, Zilmar::CONTROL Controls[4])
{
	...
	//Debug("PJ64L_XInput -> InitControllers\n");
	MainHwd = hMain;
...
{
	MessageBoxA(hParent, ABOUT "\n\nBy: KrossX/PJ64LegacyTeam © 2024", "About...", MB_OK);
}
...
{
	//Debug("PJ64L_XInput -> RomOpen\n");
	//GimmeConsole();
	GetWindowTextA(MainHwd, title, 256);

	char tmp_title[257];
	if (b_Rumble)
		sprintf_s(tmp_title, 256, "%s    Rumble", title);
	else
		sprintf_s(tmp_title, 256, "%s    Mempak", title);

	SetWindowTextA(MainHwd, tmp_title);
...
{
	SetWindowTextA(MainHwd, title);
	//Debug("PJ64L_XInput -> RomClosed\n");
	Input::Pause(true);
...
DllExport void CALL ControllerCommand(s32 port, u8* cmd)
{
	...
	//Debug("PJ64L_XInput -> ControllerCommand\t(%2d) | %02X\n", port, cmd[2]);
...
{
	if (port < 0 || cmd == NULL) return;
	//Debug("PJ64L_XInput -> ReadController\t(%2d) | %02X\n", port, cmd[2]);
